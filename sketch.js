...
extra5_snakeImg = loadImage('extra2_snake.png')
starsimg = loadImage('stars.png');
stars2img = loadImage('stars2.png');
powerUpImg = loadImage('power up.png');
}
...
function setup() {
   ...
   extra5_snake = createSprite(displayWidth/2-200,displayHeight/2-1500);
   extra5_snake.addImage(extra2_snakeImg);
   ...
   extra5_snake.velocityX = random(-3,3);
   extra5_snake.velocityY = random(3,-3);
...
function draw() {
  ...
  extra5_snake.bounceOff(border1);
  extra5_snake.bounceOff(border2);
  extra5_snake.bounceOff(border3);
  extra5_snake.bounceOff(border4);
  ...
  if(score>= 350){
     powerUp = createSprite(displayWidth/2+500,displayHeight/2+500);
     powerUp.addImage(powerUpImg)
  }

  if(snakeGroup.collide(powerUp)){
    gameState = "middle";
    text("Time left : "+time,snake.x,snake.y-130)
    powerUp.destroy();
  }

  if(stars1Group.isTouching(snake)){
    snake.scale = snake.scale+0.012;
  ...
  if(snake.collide(extra4_snake)){
   ...
   // bg1.addImage(backgroundImg)
    textSize(30);
    text("Your final score is "+score,snake.x,snake.y);
  }

  if(gameState === "middle"){
    snake.setCollider("circle",0,0,200);
    time--;
    if(stars2Group.isTouching(snake)){
      snake.scale = snake.scale+0.012;
      stars2Group.destroyEach();
...
function stars1(){
  if(frameCount%50 == 0 || frameCount%90 == 0 || frameCount%40===0){
